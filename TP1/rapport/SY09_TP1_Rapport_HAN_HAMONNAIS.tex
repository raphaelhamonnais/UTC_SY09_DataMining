...
\subsection{Reconstitution de la matrice originelle $X$ centrée en colonne}
...
\begin{itemize}
	\item $\sum\limits_{\alpha=1}^k = c_\alpha u^T$ avec $k=1$~: matrice $X$ d'origine exprimée en fonction du $1^{er}$ axe factoriel
	\item $\sum\limits_{\alpha=1}^k = c_\alpha u^T$ avec $k=2$~: matrice $X$ d'origine exprimée en fonction des 2 premiers axes factoriels
	\item $\sum\limits_{\alpha=1}^k = c_\alpha u^T$ avec $k=3$~: matrice $X$ d'origine exprimée en fonction des 3 premiers axes factoriels
	\item $\sum\limits_{\alpha=1}^k = c_\alpha u^T$ avec $k=4$~: matrice $X$ d'origine
\end{itemize}


\textcolor{red}{
\sout{La somme $\sum\limits_{\alpha=1}^k = c_\alpha u^T$ est donc égale à la matrice centrée en colonne $X$ d'origine exprimée en fonction des $k$ premiers axes factoriels de l'ACP. Ces axes factoriels sont au final une combinaison linéaire des variables d'origines. On exprime donc les individus en fonction d'une multiplication par un scalaire de chaque variable d'origine (un scalaire pour chaque variable).}}\\
\textcolor{OliveGreen}{TODO Normalement on devrait reconstruire la matrice X d'origine colonne par colonne}
